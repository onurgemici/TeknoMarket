...
Task<IEnumerable<Catalog>> GetCatalogsAsync();
 ...
 //   Task<List<CatalogListResponse>> GetCatalogsAsync();
...
public class CatalogsService : ICatalogsService
{
    ...
    //public async Task<List<CatalogListResponse>> GetCatalogsAsync()
    //{
    //    return await context.Catalogs
    //        .Select(p => new CatalogListResponse(p.Id, p.Name)).ToListAsync();
    //}
}